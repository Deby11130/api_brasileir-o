...
pprint(dados2018['fases']['2700']['jogos']['id']['102097'])
pprint(dados2018['fases']['2700']['jogos']['id']['102099'])
pprint(dados2018['fases']['2700']['jogos']['id'])
...
def ids_dos_melhor_classificados(dados,numero_de_times):
    ...
    lista_indices = range(numero_de_times) 
    for indice in lista_indices:
        resposta.append(lista_classificados[indice])
    return resposta
...
def nomes_dos_times_de_um_jogo(dados, id_jogo):
    ...
    nome_time1 = dados['equipes'][time1]['nome-comum']
    nome_time2 = dados['equipes'][time2]['nome-comum']
...
def datas_de_jogo(dados):
    lista_jogos = dados['fases']['2700']['jogos']['data']
    datas = []  
    for data in lista_jogos:
        datas.append(data)
    return datas 
...
def data_de_um_jogo(dados, id_jogo):
    if 'fases' in dados and '2700' in dados['fases'] and 'jogos' in dados['fases']['2700']:
...
def dicionario_id_estadio_e_nro_jogos(dados):
    estadios = {}
    nmrJogos = dados['fases']['2700']['jogos']['id']
    for id in nmrJogos:
        id_estadio = nmrJogos[id]['estadio_id']
...
def busca_imprecisa_por_nome_de_time(dados,nome_time):
    ...
    times=dados['equipes']
    for id in times:
        if nome_time in times[id]['nome']:
            listIds.append(id)
        elif nome_time in times[id]['nome-comum']:
            listIds.append(id)
        elif nome_time in times[id]['nome-slug']:
            listIds.append(id)
        elif nome_time in times[id]['sigla']:
            listIds.append(id)
    return listIds    
...
def datas_de_jogos_de_um_time(dados,nome_time):
    ...
    listDatas = []
    dataJogo = dados['fases']['2700']['jogos']['id']
    for id_jogo in dataJogo:
        if time_id in dataJogo[id_jogo]['time1']:
            listDatas.append(dataJogo[id_jogo]['data'])
        elif time_id in dataJogo[id_jogo]['time2']:
            listDatas.append(dataJogo[id_jogo]['data'])
    return listDatas
...
def dicionario_de_gols(dados):
    golstotais={}
    for times in dados['equipes']:
        golstotais[times]=0

    for iddojogo in dados['fases']['2700']['jogos']['id']:
        timecasa=dados['fases']['2700']['jogos']['id'][iddojogo]['time1']
        timefora=dados['fases']['2700']['jogos']['id'][iddojogo]['time2']
        golscasa=int(dados['fases']['2700']['jogos']['id'][iddojogo]['placar1'])
        golsfora=int(dados['fases']['2700']['jogos']['id'][iddojogo]['placar2'])
        golstotais[timecasa]+=golscasa
        golstotais[timefora]+=golsfora
...
def time_que_fez_mais_gols(dados):
    ...
    maxkey=max(golstotais,key=golstotais.get)

    return maxkey

print(time_que_fez_mais_gols(dados2018))
...
def rebaixados(dados):
    ...
    qtdrebaixamento=dados['fases']['2700']['faixas-classificacao']['classifica3']['faixa']
    primeirorebaixado=int(qtdrebaixamento.split('-')[0])
    timesrebaixados=dados['fases']['2700']['classificacao']['grupo']['Único'][primeirorebaixado-1:]
